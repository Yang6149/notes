...
epoll_ctl() 用于向内核注册新的描述符或者是改变某个文件描述符的状态。**已注册的描述符在内核中会被维护在一棵红黑树上**，内核中断处理程序通过**回调函数**会将 I/O 准备好的描述符加入到一个就绪链表中管理，进程调用 epoll_wait() 便可以得到事件完成的描述符。内核在把网卡上的数据copy到内核中后就来把socket插入到准备就绪链表里了。
